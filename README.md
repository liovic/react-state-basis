...
Basis analyzes runtime behavior to catch architectural violations that static linters miss:

- **⚡ Prime Movers (Root Causes)** - Pinpoints the specific hook or event that starts a chain reaction of re-renders.
- **⚡ Split State Updates** - Detects when a single user interaction forces simultaneous updates across different files (Tearing risk).
- **Ω Context Mirroring** - Local state that redundanty copies Global Context.
- **♊ Duplicate State** - Independent variables that always update in sync and should be merged.
- **🛑 Infinite Loops** - Recursive updates that will freeze the browser (with auto-kill switch).
...
### Architectural Health Report (v0.6)
...
Basis is built on heuristics inspired by **Signal Processing**, **Linear Algebra**, and **Graph Theory**.  To understand the underlying math/tech, visit the [**Full Wiki**](https://github.com/liovic/react-state-basis/wiki).
